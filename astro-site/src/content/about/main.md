---
seo:
  title: "Quiénes Somos | JDV Abogados"
  description: "JDV Abogados es un estudio jurídico con 8 áreas de práctica y cobertura nacional e internacional. Conozca a nuestro equipo de abogados comprometidos con su defensa."

hero:
  badge:
    icon: "fas fa-scale-balanced"
    text: "Estudio Jurídico"
  title: "Quiénes Somos"
  subtitle: ""
  foundationText: "JDV Abogados es un estudio jurídico con cobertura nacional e internacional, fundado por Jacqueline Del Valle Inostroza. Nuestro equipo de abogados brinda asesoría y defensa legal en diversas áreas del derecho, con un compromiso ético, transparente y profundamente humano. Los invitamos a conocernos y confiar en nosotros."
  stats:
    - icon: "fas fa-gavel"
      number: "8"
      label: "Áreas de Práctica"
      suffix: ""
    - icon: "fas fa-map-marked-alt"
      number: "16"
      label: "Regiones de Chile"
      suffix: ""
    - icon: "fas fa-hand-holding-heart"
      number: "100"
      label: "% Dedicación"
      suffix: ""
  cta:
    primary:
      text: "Agendar Consulta"
      icon: "fab fa-whatsapp"
    secondary:
      text: "Conoce al Equipo"
      icon: "fas fa-users"
      href: "#team"
  image: "/foto quienes somos JDV.webp"

mission:
  title: "Nuestra Misión"
  text: "Defendemos a quienes enfrentan la vulneración de sus derechos frente al Estado y en conflictos legales complejos.\n\nCon cobertura nacional y atención remota internacional, brindamos asesoría jurídica de excelencia."

vision:
  title: "Nuestra Visión"
  text: "Equilibrar la balanza entre las personas y las complejidades del sistema legal, garantizando acceso a una defensa de excelencia sin importar la naturaleza del conflicto."

values:
  - icon: "fas fa-briefcase"
    title: "8 Áreas de Práctica"
    description: "Defensa Estatutaria, Administrativa, Penal, Civil, Familia, Inmobiliaria y más"
    detail: "Cobertura integral para diversas necesidades legales"
  - icon: "fas fa-users"
    title: "Equipo de Abogados"
    description: "Profesionales comprometidos con cada caso"
    detail: "Trabajo colaborativo bajo la dirección de Jacqueline Del Valle"
  - icon: "fas fa-globe-americas"
    title: "Cobertura Nacional e Internacional"
    description: "Atención en todo Chile y servicios remotos para el extranjero"
    detail: "Presencial en Chile, remoto para empresas y personas en cualquier parte del mundo"
  - icon: "fas fa-hand-holding-heart"
    title: "Compromiso Humano"
    description: "Trato cercano, ético y transparente en cada gestión"
    detail: "Cada caso representa una persona y su situación particular"

founderProfile:
  title: "Jacqueline Del Valle Inostroza"
  subtitle: "Abogada Fundadora"
  badge: "35 años de trayectoria en CGR"
  bio: "Fundó JDV Abogados con la misión de poner su vasto conocimiento institucional al servicio de quienes enfrentan al Estado."
  expertise:
    title: "Áreas de Expertise"
    items:
      - "Derecho Administrativo"
      - "Derecho Público"
      - "Derecho Estatutario"
      - "Compras Públicas"
      - "Juicios de Cuentas"
      - "Acciones de Fiscalización de Control Externo"
  values:
    text: "Un profundo sentido de servicio, un actuar probo, transparente, responsable y profundamente social y humano conforman los sellos del Estudio Jurídico."

timeline:
  title: "Trayectoria de la Fundadora"
  subtitle: "35 años de carrera en la Contraloría General de la República (1990-2025)"
  milestones:
    - period: "1994-2009"
      title: "Abogada Jefe"
      location: "Contraloría Regional de Magallanes y la Antártica Chilena"
    - period: "2010-2012"
      title: "Abogada Jefe"
      location: "Comité II División de Toma de Razón y Registro, CGR Santiago"
    - period: "2012-2017"
      title: "Contralora Regional"
      location: "Región de Aysén del General Carlos Ibáñez del Campo"
    - period: "2018-2020"
      title: "Contralora Regional"
      location: "Región de Arica y Parinacota"
    - period: "2020-2024"
      title: "Jefe de Unidad"
      location: "Personal y Responsabilidad Administrativa, I Contraloría Regional Metropolitana"
    - period: "2024-2025"
      title: "Abogada Senior"
      location: "Unidad Jurídica, II Contraloría Regional Metropolitana"
    - period: "2025"
      title: "Abogada Coordinadora"
      location: "Área Jurídica, Unidad de Personal de la Administración del Estado, Contraloría Regional Metropolitana"

team:
  title: "Nuestro Equipo"
  subtitle: "Abogados comprometidos con su defensa"
  structureTitle: "Estructura del Estudio"
  structureDescription: |
    La fundadora dirige personalmente las áreas principales del estudio: Defensa Estatutaria, Defensa Administrativa e Inmobiliaria y Copropiedad, donde su experiencia de 35 años en la Contraloría General de la República constituye un diferenciador relevante.

    En las áreas complementarias, el estudio cuenta con la colaboración de abogados asociados especializados. Pablo Inostroza Vallejos aporta su expertise en Derecho Civil, Defensa Penal, Migración y Extranjería, y Familia y Menores. María Alejandra López Bertín se especializa en Derecho Civil, Derecho Laboral, Sucesión por causa de muerte, y Familia y Menores.

    Esta estructura permite ofrecer cobertura integral manteniendo el rigor técnico y la atención personalizada que caracterizan al estudio.

cta:
  title: "¿Necesita Asesoría Legal?"
  text: "Nuestro equipo de abogados está listo para atender su caso. Contamos con 8 áreas de práctica y cobertura nacional e internacional."
  button:
    text: "Agendar Consulta"
    icon: "fab fa-whatsapp"
---
