---
slug: "familia-menores"

seo:
  title: "Derecho de Familia y Menores | JDV Abogados"
  description: "Representación en divorcios, tuiciones, pensiones de alimentos, adopciones y vulneración de derechos de niños. Asesoría ética y humana en Chile."
  canonical: "/areas-practicas/familia-menores"

hero:
  badge: "Derecho de Familia · Enfoque Humano"
  h1: "Familia y Menores: Protección de Vínculos y Derechos"
  breadcrumbTitle: "Familia y Menores"
  tagline: "Representación jurídica en conflictos familiares con enfoque en el interés superior del niño y la dignidad de las personas"
  image: "/areas/familia-menores.webp"

trust:
  - value: "35+"
    label: "Años Experiencia"
  - value: "100%"
    label: "Confidencialidad"
  - value: "16"
    label: "Regiones Cubiertas"

target:
  - icon: "👨‍👩‍👧"
    title: "Padres en Proceso de Separación"
    description: "Enfrentas un divorcio o separación y necesitas resolver el cuidado personal, régimen de visitas y pensión de alimentos de tus hijos."
  - icon: "👶"
    title: "Abuelos o Familiares"
    description: "Buscas obtener el cuidado personal de un menor o regularizar un régimen de relación directa y regular con tus nietos."
  - icon: "🛡️"
    title: "Menores en Situación de Vulneración"
    description: "Un niño, niña o adolescente enfrenta situación de riesgo o vulneración de derechos y requiere protección judicial."
  - icon: "⚖️"
    title: "Demandado por Alimentos"
    description: "Has sido demandado por pensión de alimentos y necesitas representación para defender tus derechos y capacidad económica."

painPoints:
  - "Incertidumbre sobre cómo proteger a tus hijos durante un proceso de separación"
  - "Desconocimiento de tus derechos como padre, madre o familiar en el sistema de familia"
  - "Angustia ante la posibilidad de perder el contacto regular con tus hijos"
  - "Conflictos de alta carga emocional que dificultan tomar decisiones racionales"
  - "Falta de información clara sobre los procedimientos ante Tribunales de Familia"
  - "Preocupación por el bienestar de menores en situación de vulnerabilidad"

objective:
  text: "Nuestro objetivo es que cada cliente reciba representación jurídica rigurosa y humana en los momentos más difíciles de la vida familiar. Actuamos priorizando siempre el interés superior del niño, con estándares de ética, confidencialidad y respeto por la dignidad de todas las partes involucradas."
  commitments:
    - "Enfoque centrado en el interés superior del niño en todas las actuaciones"
    - "Estrategia jurídica orientada a soluciones colaborativas cuando sea posible"
    - "Comunicación periódica sobre hitos del caso y escenarios procesales"
    - "Carga controlada de causas: atención personalizada en cada etapa del proceso"

results:
  - icon: "👨‍👩‍👧"
    title: "Gestión de Régimen de Cuidado"
    description: "Representación orientada a establecer o modificar el cuidado personal y régimen de relación directa y regular."
  - icon: "💰"
    title: "Gestión de Pensiones de Alimentos"
    description: "Intervención orientada a fijar, aumentar, reducir o cesar pensiones alimenticias según las circunstancias del caso."
  - icon: "🛡️"
    title: "Protección de Menores"
    description: "Gestión orientada a obtener medidas de protección para niños, niñas y adolescentes en situación de vulnerabilidad."
  - icon: "⚖️"
    title: "Resolución de Conflictos Familiares"
    description: "Representación orientada a resolver divorcios, liquidaciones de sociedad conyugal y otros conflictos patrimoniales familiares."

servicesIncluded:
  - icon: "💔"
    title: "Divorcios y Separaciones"
    description: "Tramitación de divorcios de mutuo acuerdo y unilaterales, separación judicial y nulidades matrimoniales."
  - icon: "👶"
    title: "Cuidado Personal y Tuición"
    description: "Demandas de cuidado personal, modificación de tuición y entrega inmediata de menores."
  - icon: "👥"
    title: "Relación Directa y Regular"
    description: "Establecimiento y regulación del régimen de visitas para padres, abuelos y otros familiares."
  - icon: "💵"
    title: "Pensiones de Alimentos"
    description: "Demandas de alimentos, aumento, rebaja, cese y cumplimiento forzado de pensiones alimenticias."
  - icon: "🛡️"
    title: "Medidas de Protección"
    description: "Solicitud de medidas cautelares y de protección para menores en situación de riesgo o vulneración."
  - icon: "📋"
    title: "Adopción y Filiación"
    description: "Asesoría y representación en procesos de adopción, reconocimiento de paternidad e impugnación de filiación."

process:
  - step: 1
    title: "Evaluación Inicial"
    description: "Analizamos tu situación familiar, identificamos las necesidades jurídicas y explicamos las opciones disponibles."
  - step: 2
    title: "Definición de Estrategia"
    description: "Diseñamos el plan de acción priorizando soluciones colaborativas o preparando la vía judicial según corresponda."
  - step: 3
    title: "Representación y Gestión"
    description: "Actuamos ante Tribunales de Familia, audiencias de mediación y gestiones prejudiciales con preparación estratégica."
  - step: 4
    title: "Seguimiento y Cumplimiento"
    description: "Acompañamos hasta la resolución definitiva y velamos por el cumplimiento efectivo de sentencias y acuerdos."

faq:
  - question: "¿Es obligatoria la mediación antes de demandar en materias de familia?"
    answer: "Sí, en la mayoría de los casos. La Ley exige mediación previa obligatoria en materias de alimentos, cuidado personal y relación directa y regular. Solo se exceptúan casos de violencia intrafamiliar o vulneración grave de derechos."
  - question: "¿La información que comparto sobre mi situación familiar es confidencial?"
    answer: "Sí, absolutamente. Todo lo que nos compartas está protegido por el secreto profesional. Además, los procedimientos de familia tienen carácter reservado por ley, protegiendo especialmente la intimidad de los menores involucrados."
  - question: "¿Qué significa el interés superior del niño?"
    answer: "Es el principio rector del derecho de familia chileno. Significa que en toda decisión judicial o administrativa que afecte a menores, debe priorizarse su bienestar integral por sobre los intereses de los adultos."
  - question: "¿Puedo modificar una sentencia de alimentos o tuición ya dictada?"
    answer: "Sí, cuando existan circunstancias nuevas que lo justifiquen. Las sentencias de familia no producen cosa juzgada material en estas materias, permitiendo su revisión ante cambios relevantes."
  - question: "¿Qué pasa si el otro padre no cumple el régimen de visitas o la pensión?"
    answer: "Existen mecanismos de cumplimiento forzado. En pensiones: retención judicial, arresto y arraigo. En régimen de visitas: apercibimientos, multas y eventualmente modificación del cuidado personal."
  - question: "¿Atienden casos en regiones o solo en Santiago?"
    answer: "Atendemos casos a nivel nacional. Nuestro equipo tiene experiencia en distintas zonas del país y trabajamos presencial o remoto según tu necesidad."

finalCta:
  headline: "¿Enfrentas un conflicto familiar? Busquemos soluciones."
  text: "Los asuntos de familia requieren sensibilidad y rigor jurídico. Nuestro equipo te acompaña con ética, confidencialidad y un enfoque centrado en proteger lo que más importa. Agenda una consulta."
  button: "Agendar Consulta"
---

## Derecho de familia con perspectiva humana

Los conflictos familiares son de los más difíciles que una persona puede enfrentar. Nuestro equipo entiende que detrás de cada causa hay familias, niños y emociones intensas. Por eso combinamos **rigor técnico** con **sensibilidad humana** en cada representación.

> **El interés superior del niño guía todas nuestras actuaciones.** En cada estrategia, audiencia y negociación, priorizamos el bienestar de los menores involucrados, sin perder de vista los legítimos derechos de nuestros representados.

---

## Ámbito de actuación

**Representación integral:** Actuación ante Tribunales de Familia, Cortes de Apelaciones y Corte Suprema. Participación en audiencias de mediación, gestiones ante el Registro Civil y coordinación con instituciones de protección de menores.

Nuestro equipo atiende casos en **todo Chile**, con cobertura nacional para familias que requieren representación especializada.

---

## Soluciones colaborativas cuando sea posible

Privilegiamos los **acuerdos y soluciones colaborativas** por sobre la judicialización innecesaria. Cuando las partes pueden dialogar, orientamos hacia acuerdos que protejan a los hijos y respeten los derechos de todos. Cuando la vía judicial es inevitable, litigamos con preparación estratégica y determinación.
