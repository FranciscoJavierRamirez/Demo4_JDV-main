---
sectionTitle: "Servicios Pro Bono"
sectionSubtitle: "Comprometidos con el acceso a la justicia para todos"

intro:
  icon: "fas fa-heart"
  text: "En JDV & Abogados creemos en el acceso equitativo a la justicia. Por eso, ofrecemos servicios legales **Pro Bono** (gratuitos) para personas que califiquen según criterios socioeconómicos."

criteriaTitle: "¿Calificas para Servicios Pro Bono?"

criteria:
  - icon: "fas fa-check-circle"
    text: "Funcionarios públicos con recursos económicos limitados"
  - icon: "fas fa-check-circle"
    text: "Personas adultas mayores en situación de vulnerabilidad"
  - icon: "fas fa-check-circle"
    text: "Casos de especial relevancia social o defensa de derechos fundamentales"
  - icon: "fas fa-check-circle"
    text: "Organizaciones sin fines de lucro (fundaciones animalistas, asociaciones comunitarias)"

cta:
  question: "¿Crees que calificas?"
  description: "Consulta por nuestros servicios Pro Bono. Evaluaremos tu caso y te informaremos si cumples los requisitos."
  buttonText: "Consultar por Pro Bono"
  buttonAction: "openProBonoModal()"

visualCards:
  - icon: "fas fa-balance-scale-left"
    title: "Justicia Accesible"
    text: "Todos merecen defensa legal de calidad, independiente de su situación económica"
  - icon: "fas fa-hands-helping"
    title: "Compromiso Social"
    text: "35 años de servicio público nos enseñaron el valor de ayudar a quienes más lo necesitan"
  - icon: "fas fa-shield-alt"
    title: "Misma Calidad"
    text: "Los casos Pro Bono reciben la misma dedicación y profesionalismo que cualquier otro"
---
